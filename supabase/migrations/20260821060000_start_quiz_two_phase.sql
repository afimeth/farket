-- start_quiz v2: restructures the quiz into two explicit 5-question phases.
--
-- Phase 1 (positions 1-5): act-1 template pool only, now targeting 2 easy +
-- 2 medium + 1 hard (was 3+3+1 across 7 questions) — same tolerant
-- cross-fill/backfill strategy as the previous version, just with smaller
-- per-tier targets and a 5-question floor instead of 7.
--
-- Phase 2 (positions 6-10), filled in priority order:
--   1. exactly 1 profile-owner-authored custom_questions row (unchanged
--      selection logic, just moved earlier from position 10).
--   2. 1-2 questions auto-generated from the target's quiz-eligible numeric
--      profile_identity_attributes rows (height_cm/weight_kg/age), via the
--      new pick_numeric_distractors() helper.
--   3. remaining slots (until position 10) filled from the existing act-2
--      hard template pool, same query as before but with a dynamic limit.
