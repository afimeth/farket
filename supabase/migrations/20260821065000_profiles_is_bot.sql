-- Discovery cards need to show a "test bot" badge for seeded bot accounts, but there is no
-- schema-level marker distinguishing them from real users (only the @bot.farket.local email
-- convention and a hardcoded username list in seed_bot_profiles.py). Add a real column and
-- surface it from discover_profiles.

alter table public.profiles add column is_bot boolean not null default false;

-- CREATE OR REPLACE copied verbatim from the previous latest definition
-- (20260821010000_discover_profiles_photo_list.sql) — only the new 'is_bot' key was inserted
-- into the jsonb_build_object.
